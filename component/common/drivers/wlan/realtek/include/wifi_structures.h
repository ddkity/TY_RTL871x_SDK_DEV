...
typedef struct rtw_rx_info {
	...
	uint8_t filter;		// 1: HT-20 2T and not LDPC pkt; 2: HT-40 2T and not LDPC pkt; 3: LDPC pkt
	signed char rssi;	// -128~-1
	uint16_t channel;	// channel whick this pkt in
	uint8_t agg:1;		// aggregation pkt or not. If an AMPDU contains only one MPDU then above 'length' is the antual pkt length without FCS, buuut if it contains multiple MPDUs then above 'length' is useless because it cannot tell how many MPDUs are contained and how long is each MPDU.
	uint8_t mcs:7;		// mcs index
}rtw_rx_info_t;
