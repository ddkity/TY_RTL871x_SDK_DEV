...
STATIC VOID get_light_data(VOID)
{
	...
	switch(dp_data.WORK_MODE){
		case WHITE_MODE:
            ...
            //printf("FIN_WHITE_VAL = %d, FIN_WARM_VAL = %d, BRIGHT = %d\n\n", light_data.FIN_WHITE_VAL, light_data.FIN_WARM_VAL, dp_data.BRIGHT);
...
VOID light_data_handler(TY_OBJ_DP_S root)
{
    ...
    switch(dpid) {
        case DPID_1:
            PR_INFO("{\"%d\":\"%d\"}", dpid, root.value.dp_bool);
			switch(root.value.dp_bool) {
			    case cJSON_False:
					...
					switch(dp_data.WORK_MODE)
					{
						...
						case SCENE_MODE:
							MutexUnLock(flash_scene_handle.mutex);
							start_gra_change(NORMAL_DELAY_TIME);    /* huangjituan 渐变的关灯 */
							break;
						case ROUGUANG_SCENE:
				        case BANLAN_SCENE:
							gtimer_stop(&hw_timer);
							//send_light_data(light_data.RED_VAL, light_data.GREEN_VAL, light_data.BLUE_VAL, light_data.WHITE_VAL, light_data.WARM_VAL);  /* huangjituan 马上关灯 */
							send_light_data(light_data.RED_VAL, light_data.GREEN_VAL, light_data.BLUE_VAL, light_data.WHITE_VAL, 0x00);  /* huangjituan 马上关灯 */
							MutexUnLock(flash_scene_handle.mutex);
							break;
						default:
							//send_light_data(light_data.RED_VAL, light_data.GREEN_VAL, light_data.BLUE_VAL, light_data.WHITE_VAL, light_data.WARM_VAL);
							send_light_data(light_data.RED_VAL, light_data.GREEN_VAL, light_data.BLUE_VAL, light_data.WHITE_VAL, 0x00);
							MutexUnLock(flash_scene_handle.mutex);
							break;
					}
        ...
        case DPID_2:
            PR_INFO("{\"%d\":\"%d\"}", dpid, root.value.dp_enum);
        ...
        case DPID_3:
            PR_INFO("{\"%d\":\"%d\"}", dpid, root.value.dp_value);
...
#endif
        case DPID_5:
            PR_INFO("{\"%d\":\"%s\"}", dpid, root.value.dp_str);
...
STATIC VOID light_gra_change(PVOID pArg)
{
	...
	while(1)
	{
			...
			light_data.GREEN_VAL != light_data.LAST_GREEN_VAL || light_data.BLUE_VAL != light_data.LAST_BLUE_VAL)
	    {
			...
			MAX_VALUE = get_max_value(ABS(delata_red), ABS(delata_green), ABS(delata_blue), ABS(delata_white), 0x00);
			//PR_DEBUG("MAX: %d", MAX_VALUE);
			if(gra_change.scale_flag == FALSE){
				r_scale = ABS(delata_red)/1.0/MAX_VALUE;    /* 计算每种颜色应该使用的步长 */
			...
			if(delata_red != 0){
			    ...
			    {
					 light_data.LAST_RED_VAL += delata_red; /* 每次增加的步长 */
				}else{
					if(delata_red < 0)
						light_data.LAST_RED_VAL -= RED_GRA_STEP;    /* 如果剩下的值不够那么多步长，则增加剩下的步长就可以了 */
					else
			...
			if(dp_data.SWITCH == FALSE && (dp_data.WORK_MODE == ROUGUANG_SCENE || dp_data.WORK_MODE == BANLAN_SCENE)){
				...
				MutexLock(flash_scene_handle.mutex);
				if(dp_data.WORK_MODE != BINFENG_SCENE && dp_data.WORK_MODE != XUANCAI_SCENE)
					send_light_data(light_data.LAST_RED_VAL*RESO_VAL, light_data.LAST_GREEN_VAL*RESO_VAL, light_data.LAST_BLUE_VAL*RESO_VAL, light_data.LAST_WHITE_VAL*RESO_VAL, 0x00);
				MutexUnLock(flash_scene_handle.mutex);
			}
		}else{  /* 数据发送处理完成 */
			if(dp_data.WORK_MODE != ROUGUANG_SCENE && dp_data.WORK_MODE != BANLAN_SCENE){
				send_light_data(light_data.FIN_RED_VAL, light_data.FIN_GREEN_VAL, light_data.FIN_BLUE_VAL, light_data.FIN_WHITE_VAL, 0x00);
...
unsigned char pre_theard_init_done = 0;
	...
	while(1)
	{
		...
		if(dp_data.SWITCH == TRUE && flash_scene_flag == TRUE ){
			switch(dp_data.WORK_MODE){
				case ROUGUANG_SCENE:
				case BANLAN_SCENE:
                    pre_theard_init_done = 0x01;
					if(dp_data.WORK_MODE == ROUGUANG_SCENE){
						require_time = flash_light_data.SPEED*2+15; /* 变化速度 */
					}else{
...
VOID dev_reset_judge(VOID)
{
    ...
    if(rst_num == REASON_DEFAULT_RST){
        ...
        PR_NOTICE("rst_cnt+1:%d", rst_cnt+1);
...
BOOL_T power_up_count_judge_smcfg (void)
{
	if(get_reset_cnt() >= 5){
		set_reset_cnt(0);
		set_default_dp_data();
...
#if 0
static void pre_app_init_theard(void *param)
{
    #if 0
    unsigned char pre_cnt = 0;
    unsigned short pre_flag = 0;
    #endif

    while(pre_theard_init_done == 0x00)
    {
         vTaskDelay(5);
         switch(dp_data.WORK_MODE)
        {
            case ROUGUANG_SCENE:
                //send_light_data(flash_light_data.data_group[0].RED_VAL, flash_light_data.data_group[0].GREEN_VAL - pre_cnt, flash_light_data.data_group[0].BLUE_VAL,0x00,0x00);
            ...
            case BINFENG_SCENE:
                //send_light_data(flash_light_data.data_group[2].RED_VAL, flash_light_data.data_group[2].GREEN_VAL, flash_light_data.data_group[2].BLUE_VAL - pre_cnt,0x00,0x00);
                send_light_data(flash_light_data.data_group[0].RED_VAL, flash_light_data.data_group[0].GREEN_VAL, flash_light_data.data_group[0].BLUE_VAL,0x00,0x00);
                break;

            case XUANCAI_SCENE:
                //send_light_data(flash_light_data.data_group[0].RED_VAL - pre_cnt,flash_light_data.data_group[0].GREEN_VAL,flash_light_data.data_group[0].BLUE_VAL,0x00,0x00);
                send_light_data(flash_light_data.data_group[0].RED_VAL, flash_light_data.data_group[0].GREEN_VAL, flash_light_data.data_group[0].BLUE_VAL,0x00,0x00);
                break;

            case BANLAN_SCENE:
                //send_light_data(flash_light_data.data_group[1].RED_VAL,flash_light_data.data_group[1].GREEN_VAL - pre_cnt,flash_light_data.data_group[1].BLUE_VAL,0x00,0x00);
                send_light_data(flash_light_data.data_group[0].RED_VAL, flash_light_data.data_group[0].GREEN_VAL, flash_light_data.data_group[0].BLUE_VAL,0x00,0x00);
                break;
        ...
        if(pre_flag >= 0 && pre_flag <= 230){
            pre_cnt = pre_cnt + 1;
        }else if(pre_flag > 230 && pre_flag <= 460){
            pre_cnt = pre_cnt - 1;
        }else{
            pre_flag = 0;
        }
    ...
    PR_INFO("pre_app_init_theard done. exit......");
    vTaskDelete(NULL);
}
#endif
