...
static CTRL_CHANNEL channels[] = 
{
    ...
    {
        // 继电器
        .relay  = {.type = IO_DRIVE_LEVEL_HIGH, .pin = TY_GPIOA_15},
        // 按钮
        .button = {TY_GPIOA_0,TRUE,LP_INVALID,0,400,NULL},
        // 通道状态指示灯
        .led    = {.type = IO_DRIVE_LEVEL_LOW, .pin = TY_GPIOA_12},
        // 绑定dpid
        .dpid   = 1,
...
HW_TABLE g_hw_table = 
{
    ...
    .wifi_stat_led = {.type = IO_DRIVE_LEVEL_LOW, .pin = TY_GPIOA_5},
    .rst_button = {TY_GPIOA_0,TRUE,LP_ONCE_TRIG,KEY_RST_TIME,400,key_process},
    .channels = channels
};
...
OPERATE_RET init_hw(HW_TABLE *hw_table)
{
	...
	UCHAR_T i;	
	if(hw_table->wifi_stat_led.type == IO_DRIVE_LEVEL_HIGH){  //初始化 熄灭led
...
OPERATE_RET prod_test_init_hw(HW_TABLE *hw_table)
{
	...
	tuya_set_led_light_type(hw_table->wifi_stat_led_handle,OL_FLASH_HIGH,250,LED_TIMER_UNINIT);//250ms flash    lql
    ...
    hw_table->rst_button.call_back = Protest_key_process;        //lql
    if(hw_table->rst_button.call_back != NULL){
        op_ret = reg_proc_key(&hw_table->rst_button);//初始化reset按键
    	if( op_ret != OPRT_OK){
    		PR_ERR("reg_proc_key err:%d",op_ret);
