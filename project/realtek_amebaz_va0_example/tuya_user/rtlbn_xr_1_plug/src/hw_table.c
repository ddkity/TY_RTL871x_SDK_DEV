...
//按键重置去抖
extern volatile UCHAR Boot_key;
...
OPERATE_RET init_hw(HW_TABLE *hw_table)
{
	...
	for(i = 0; i < hw_table->channel_num; i++) {
        ...
        if(hw_table->channels[i].relay.type != IO_DRIVE_LEVEL_NOT_EXIST) {//普通输出io
			...
			if(hw_table->channels[i].relay.type == IO_DRIVE_LEVEL_HIGH) {
				tuya_gpio_write(hw_table->channels[i].relay.pin,false);
			}else{
				tuya_gpio_write(hw_table->channels[i].relay.pin,true);
			}
