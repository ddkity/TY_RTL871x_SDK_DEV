...
#define ELE_SLEEP_TIME 2//电量统计上层采样间隔
#define _IS_OVER_PROTECT 0//是否存在过流保护功能
#define LIMIT_CURRENT 10000//过流保护限值，单位1mA
#define _IF_REPT_COE   TRUE  //是否上报电量校准系数

#define TAR_VOL 2200         //0.1V
#define TAR_CURR 392        //mA
#define TAR_POWER 864        //0.1W
#define TAR_RANGE_V 10         //%
#define TAR_RANGE_I 20         //%
...
OPERATE_RET app_dltj_init(APP_DLTJ_MODE mode)
{
    ...
    if(mode == APP_DLTJ_NORMAL){
        op_ret= pl7211_init(PL_INIT_NORMAL);
        if(OPRT_OK != op_ret){
            PR_ERR("pl7211 init err", op_ret);
        }
        
        mAppDltj.app_dltj_info.edpid = 3;
        mAppDltj.app_dltj_info.idpid = 4;
        mAppDltj.app_dltj_info.pdpid = 5;
        mAppDltj.app_dltj_info.vdpid = 6;
        //set_tem_ele_val(100);
        //addto_local_data(1000,1234);
        ...
        op_ret = pl7211_init(PL_INIT_PRODTEST);
        if(OPRT_OK != op_ret){
            PR_ERR("mode:%d,dltj prod test err!!!",mode);
            return op_ret;
        }
        SystemSleep(3100);
        get_ele_par(&power_cnt.cur_power,&power_cnt.cur_vol,\
        &power_cnt.cur_current);
        if(value_range_judge(power_cnt.cur_vol, TAR_VOL , TAR_RANGE_V) && \
        value_range_judge(power_cnt.cur_current, TAR_CURR , TAR_RANGE_I) && \
        value_range_judge(power_cnt.cur_power, TAR_POWER, TAR_RANGE_P)){
            return OPRT_OK;
        }else{
...
{ 
    ...
    while(1){
        ...
        add_ele_handle();
           ...
           if(power_cnt.cur_current>=20000){
        ...
        PR_DEBUG(".........................................mem size:%d",SysGetHeapSize());
    }
...
STATIC OPERATE_RET set_tem_ele_val(INT_T val)
{
    ...
    cJSON_Delete(root);
    PR_DEBUG("set tem ele %s", buf);
    op_ret = wd_common_write(TEM_ELE_SAVE_KEY, buf, strlen(buf));
	if(OPRT_OK != op_ret) {
		APP_DLTJ_DEBUG("set tem ele err!!!:%d",op_ret);
        Free(buf);
		return op_ret;
	}
...
STATIC OPERATE_RET get_tem_ele_val(INT_T *val)
{
	...
	UCHAR *buf = NULL;
	PR_DEBUG("before get tem ele mem size:%d",SysGetHeapSize());
    UINT_T buf_len;
    op_ret = wd_common_read(TEM_ELE_SAVE_KEY, &buf,&buf_len);
	if(OPRT_OK != op_ret) {
    ...
    *val = json->valueint;
	...
	PR_DEBUG("after get tem ele mem size:%d",SysGetHeapSize());
    return OPRT_OK;
...
STATIC OPERATE_RET set_ele_data(VOID)
{
	...
	PR_DEBUG("set data buf:%s",buf);
	
    op_ret = wd_common_write(ELE_SAVE_KEY, buf, strlen(buf));
	if(OPRT_OK != op_ret) {
		APP_DLTJ_DEBUG("msf_get_single err:%d",op_ret);
        cJSON_Delete(array);
        Free(buf);
...
STATIC OPERATE_RET get_ele_data(VOID)
{
	PR_DEBUG("before get ele mem size:%d",SysGetHeapSize());
	OPERATE_RET op_ret;
    ...
    UINT_T buf_len;
    op_ret = wd_common_read(ELE_SAVE_KEY, &buf, &buf_len);
	...
	PR_DEBUG("get ele data buf:%s",buf);
	...
	cJSON_Delete(array);
	PR_DEBUG("after get ele mem size:%d",SysGetHeapSize());
...
STATIC INT_T report_cur_data(VOID)
{
    ...
    if(!value_range_judge(power_cnt.cur_current,power_cnt.last_current,REPT_THRE_CURR) ||\
       ...
       !value_range_judge(power_cnt.cur_vol,power_cnt.last_vol,REPT_THRE_VOL)){
        snprintf(dpid_str,8, "%d", mAppDltj.app_dltj_info.idpid);
        cJSON_AddNumberToObject(root,dpid_str,power_cnt.cur_current);
        snprintf(dpid_str,8, "%d", mAppDltj.app_dltj_info.pdpid);
    	cJSON_AddNumberToObject(root,dpid_str,power_cnt.cur_power);
    	snprintf(dpid_str,8, "%d", mAppDltj.app_dltj_info.vdpid);
