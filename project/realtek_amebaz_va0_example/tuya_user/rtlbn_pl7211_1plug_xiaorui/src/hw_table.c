...
STATIC VOID wifi_light_direct_relay();

STATIC en_wifi_show_relay = FALSE;
...
static CTRL_CHANNEL channels[] = 
{
    // channel 0
    {
        ...
        // 按钮
        .button = {TY_GPIOA_5,TRUE,LP_INVALID,0,400,NULL},
        // 通道状态指示灯
...
HW_TABLE g_hw_table = 
{
    ...
    .rst_button = {TY_GPIOA_5,TRUE,LP_ONCE_TRIG,KEY_RST_TIME,400,key_process},
    .channels = channels
};
    ...
    if(g_hw_table.rst_button.port== port){
        if(LONG_KEY == type) {
            ...
            if(g_hw_table.channels[0].stat){
                snprintf(buff,30, "{\"%d\":true}", g_hw_table.channels[0].dpid);
            }else{
                snprintf(buff,30, "{\"%d\":false}", g_hw_table.channels[0].dpid);
            }
            dp_upload_proc(buff);
            if( g_hw_table.channels[0].cd_sec != -1){
                // 上报通道倒计时状态
                snprintf(buff,30, "{\"%d\":0}",  g_hw_table.channels[0].cd_dpid);
                dp_upload_proc(buff);
...
STATIC VOID wifi_light_direct_relay()
{
    if(en_wifi_show_relay){
        if(g_hw_table.channels[0].stat == TRUE){
