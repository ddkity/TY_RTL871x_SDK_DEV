...
STATIC OPERATE_RET upgrade_file_cb(IN CONST BYTE_T *data, IN CONST UINT len)
{	
	UCHAR_T head_buf[10];
	UCHAR_T sum_check = 0;
	UCHAR_T icnt = 0, i = 0;
...
STATIC VOID recv_func_test(VOID)
{
    ...
    BOOL flag = TRUE;
    OPERATE_RET op_ret = OPRT_OK;
	#if 1 //make sure that already get uuid and auth_key before func_test
	// gateway base info read
...
{  
	...
	if( get_gw_nw_status() != GNS_WAN_VALID ) {//lql
	    //PR_ERR("wifi not connect to cloud");
		return;
	}
...
//55 aa 03 07       00 07     02 00 04 00 00 00 1e       34
	...
	while((len - offset) >= TY_DP_FRAME_MIN) {
		...
		case DP_RAW: {
            ...
            op_ret = dev_report_dp_raw_sync(get_gw_cntl()->dev->dev_if.id,dp_data->dpid,dp_data->data,dp_data->len,5000);
...
{
    UCHAR_T count = ty_uart_read_data(TY_UART0,ty_msg.proc_buf + ty_msg.rcount, HN_RECV_BUF_MAX - ty_msg.rcount);
	ty_msg.rcount += count;
	if( ty_msg.rcount > 0 ) {
...
VOID dev_ug_inform_cb(IN CONST CHAR_T *dev_id,IN CONST FW_UG_S *fw)
{
    ...
    PR_DEBUG("fw->sw_ver:%s", fw->sw_ver);
    PR_DEBUG("fw->file_size:%d", fw->file_size);
    tuya_iot_upgrade_dev(dev_id,fw,NULL,NULL,NULL);
